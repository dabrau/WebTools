...
const constraints = (path, column, filters) => {
  return filters.map(({ comparator, value }) => {
    ...
    if (boolValues.includes(comparator)) {
      return constraint.concat([`::${comparator}`]);
    }

    if (comparator === 'has') {
      return [...path, `::${comparator}`, column];
    }
    ...
    if (Array.isArray(comparator)) {
      ...
      if (comparator.includes('in')) {
        const val = value ? value.split(',').map(s => s.trim()) : [];
        return constraint.concat([...comp, val]);
      }
...
export const constructDSLQuery = (query, graph) => {
  ...
  const dslQuery = query.reduce((acc, curr) => {
    ...
    const queryPath = pathNames.reduce((qp, tableName, ind, pNames) => {
      let tablePath = [tableName];

      let previousTable = base;
      if (pNames[ind - 1]) {
        previousTable = graph[pNames[ind - 1]];
      }

      const linkAttr = Object.values(previousTable)
        .find(col => col.model_name === tableName);

      if (linkAttr && linkAttr.attribute_class !== fk) {
        tablePath.push('::all')
      }
    ...
    if (queryPath.includes('::all')) {

      const indexesOfOneToManyTable = queryPath.reduce((indexes, elem, ind) => {
        if (elem === '::all') {
          return [...indexes, ind - 1]
        }
        return indexes
      }, []);

      var newPath = [];
      indexesOfOneToManyTable.forEach((tableInd, ind) => {
        if (ind === 0) {
          newPath = queryPath.slice(0, tableInd + 1)
        } else {
          newPath = [...newPath, ...queryPath.slice(indexesOfOneToManyTable[ind - 1] + 1, tableInd + 1)]
        }
        const table = queryPath[tableInd];
        const queryTable = query.find(t => t.name === table);
        if (queryTable) {
          const tableConstraints = queryTable.columns
            .map(col => constraints([], col.name, col.filters))
            .reduce((acc, curr) => [...acc, ...curr], []);

          newPath = newPath.concat(tableConstraints);
        }

        if (ind === indexesOfOneToManyTable.length - 1) {
          newPath = [...newPath, ...queryPath.slice(tableInd + 1)];
        }
      })
    }
    ...
    const filters = curr.columns
      .map(({ name, filters }) => constraints(queryPath, name, filters))
      .reduce((acc, curr) => [...acc, ...curr], []);

    const columns = curr.columns.map(({ name }) => {
      let path = [...(newPath || queryPath), name];
